 ...
 * instead.
 *
 * The model sees a *sample* of each field's values (the agent's
 * `simplify_data_domains` prints 8 of N, with an honest distinct count and a
 * `ListFieldValues` tool for the rest), so it still names values that don't
 * exist — a typo, a case mismatch, a value it reached for by analogy. Fields
 * also come and go: HuBMAP is renaming `assay_type` to `dataset_type`, and a
 * model asking to filter `assay_type` by "Xenium" is asking for a value that
 * is real under a field that is not. Before this module either miss was
 * dropped in `syncFiltersFromMessages` and rendered as an empty chat bubble.
 *
