...
| `VITE_UDI_READ_ONLY`       | `false`                         | Start in read-only mode: the chat collapses to a rail and every editing control (card drag, resize, rename, close, field tweak, grid settings, session import) is hidden, while brushing, filters, the table toggle and downloads stay. `true` leaves an Explore Data button over the dashboard (and one on the rail) that opens the chat; `locked` removes both. With the chat collapsed there is nothing to build a dashboard from, so pair this with `VITE_UDI_INITIAL_SESSION`. |
...
`UDIChat` with `readOnly`). The chat pane collapses to a slim sidebar rail and every editing
control disappears; the dashboard takes the full width.
...
bottom right (and a "Start chatting" button on the rail) leaves read-only, at which point the full
chat and every editing control appear — backed by whatever conversation `initialSession` carried,
so the transcript behind the dashboard is intact. If your users must not be able to chat at all,
pass `readOnly="locked"` to `UDIChat` instead: both buttons and the rail are dropped, leaving no
way out.
...
| Grid settings (columns, row height, reset)    | The chart / table toggle and all-fields |
| Session import / export                       | The info tooltips                       |
|                                               | Download Data (and any custom actions)  |
...
| `read_only_exited`           | User leaves read-only mode from the collapsed chat rail                                         | _(none)_                                                                                                                                                                                                                                                     |
...
| `visualization_tweaked`      | User changes the field a `VizTweakComponent` dropdown controls                                  | `encoding: string` — the channel, or the template parameter for a re-bind<br>`kind: 'field' \| 'dimension' \| 'measure' \| 'binding'` — `binding` means the agent re-instantiated the template; the rest are local spec rewrites                             |
| `visualization_tweak_failed` | The agent refused a template re-bind (invalid binding, unknown template, transport failure)     | `encoding: string`<br>`kind: 'binding'`                                                                                                                                                                                                                      |
