...
`udi-toolkit` is the reference implementation of the Universal Discovery Interface
(UDI) Grammar. It provides the grammar type definitions plus `UDIVis` — a component
that renders a UDI grammar spec as an interactive visualization — and a set of
headless data APIs.

The Grammar is defined with TypeScript typings in
[GrammarTypes.ts](./GrammarTypes.ts). The JSON Schema
([UDIGrammarSchema.json](./UDIGrammarSchema.json)) is generated from it via
`pnpm build-schema`.
...
From the repo root, `udi-toolkit` is consumed by the chat app and the grammar
demo app as `workspace:*` (its exports point at `dist/`), so **build the
toolkit before building or typechecking its consumers**:

```bash
pnpm build:toolkit           # from the repo root
# or, scoped to this package:
pnpm --filter udi-toolkit build:all
```

Working inside this package (`packages/grammar`):

```bash
pnpm build:all    # Builds Vue, Custom Element, and React targets
pnpm test         # Builds + runs smoke tests
pnpm build-schema # Regenerates UDIGrammarSchema.json from GrammarTypes.ts
```

### Storybook

The toolkit uses [Storybook](https://storybook.js.org/) to develop and test the
grammar with individual stories. Run `pnpm storybook` (or `pnpm dev:storybook`
from the repo root) to browse the stories at `http://localhost:6006`.
...
For an interactive spec editor built on the toolkit, see the demo app in
[`apps/grammar-app`](../../apps/grammar-app).
