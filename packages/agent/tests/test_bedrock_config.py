...
class TestBedrockPackaging:
    """The install group itself. It was once lost in a merge while everything
    above kept passing locally, and the deploy image then failed at startup."""

    _AGENT_ROOT = Path(__file__).resolve().parents[1]

    def test_pyproject_declares_the_extra(self):
        pyproject = tomllib.loads((self._AGENT_ROOT / "pyproject.toml").read_text())
        extras = pyproject["project"]["optional-dependencies"]
        assert any(dep.startswith("openai[bedrock]") for dep in extras["bedrock"])
        assert "bedrock" in extras["all"][0]

    def test_deploy_image_installs_the_extra(self):
        """deploy-agent.yml builds this Dockerfile; UDI_BEDROCK needs botocore."""
        syncs = [
            line
            for line in (self._AGENT_ROOT / "Dockerfile").read_text().splitlines()
            if "uv sync" in line
        ]
        assert syncs
        for line in syncs:
            assert "--extra bedrock" in line, line
