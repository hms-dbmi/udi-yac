...
# ---------------------------------------------------------------------------
# A tool's description must be the TOOL's, not one of its parameters'
# ---------------------------------------------------------------------------


def test_no_tool_description_is_a_parameter_blurb(generated):
    """The shadowing bug, twice now, and invisible both times.

    `_generate_join_entity_tool` built the tool description into a local called
    `description`, then reused that name for each entity parameter's blurb
    inside the loop — so what shipped as the tool's own description was "An
    additional data entity (table) to join with (entity3)." for all 19 join
    templates, every survival variant among them. Nothing failed; the model
    simply had nothing but the tool NAME to choose on, and it chose badly.

    The single-entity generator carries a comment warning about exactly this,
    which is how it happened again in the other one. So: assert it.
    """
    tool_defs, _dispatch, _templates, _tags = generated
    ...
    for tool in tool_defs:
        ...
        for param, spec in fn["parameters"]["properties"].items():
            ...
            if blurb and description.startswith(blurb[:60]):
                leaked.append(f"{fn['name']} describes itself with its {param} blurb")
    assert not leaked, "\n".join(leaked)


def test_every_tool_description_names_its_chart_type(generated):
    """`_build_tool_description` leads with `[line]`, `[barchart]`… A description
    that does not start that way did not come from the builder, which is the
    cheap tell that something overwrote it."""
    tool_defs, _dispatch, _templates, _tags = generated
    bad = [
        t["function"]["name"]
        for t in tool_defs
        if not t["function"].get("description", "").startswith("[")
    ]
    assert not bad, f"descriptions not built by _build_tool_description: {bad}"


def test_presence_and_membership_tell_each_other_apart(generated):
    """The two the model actually confuses: "did the patient receive X" picked
    the presence template, which for a table of drugs means "had any chemo".
    Each description has to hand the other case over, and say so early enough to
    survive the 1024-character budget."""
    tool_defs, dispatch, _templates, _tags = generated
    by_name = {t["function"]["name"]: t["function"] for t in tool_defs}

    presence = by_name[next(n for n in dispatch if n.endswith("_survival_presence"))]
    membership = by_name[
        next(n for n in dispatch if n.endswith("_survival_ever_matching"))
    ]

    # Presence sends a named value away.
    assert "DO NOT use it when the request names a particular value" in presence["description"]
    # Membership claims it, in the words a user types.
    assert "receive methotrexate or not" in membership["description"]
    # Both survive truncation.
    for fn in (presence, membership):
