...
It does not scope query results — server-side query backends connect with the
service credentials in `UDI_QUERY_BACKENDS`, and the token is not passed
downstream to StarRocks or DuckDB.
